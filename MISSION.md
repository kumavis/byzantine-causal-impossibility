...
| `isabelle build -D .` succeeds                                  | **Verified** on Isabelle 2025-2 + AFP snapshot `afp-2026-05-13`.  Wall time ~10s, 21 theory files at 100%, 0 `sorry`/`oops`. |
| `isabelle build -o document=pdf -D .` succeeds                  | **Verified** on the same toolchain.  Produces `document.pdf` (146 pages, A4).  A committed snapshot is at `dist/ByzantineCD.pdf`. |
